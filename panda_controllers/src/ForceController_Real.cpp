...
bool ForceController::init(hardware_interface::EffortJointInterface *hw, ros::NodeHandle &handle) {
    ...
    sub_forcetorque_sensor_ = handle.subscribe<geometry_msgs::WrenchStamped>("/franka_state_controller/F_ext", 1, &ForceController::updateFTsensor, this,ros::TransportHints().reliable().tcpNoDelay());
    ...
    std::string urdf_file = "/home/tran/catkin_ws/src/panda_controllers/model/robots/sim.urdf";
    urdf::Model model;
...
void ForceController::update(const ros::Time &time, const ros::Duration &period) {
  //// get q of current time step
   ...
   // cart_pos_current.M.GetQuaternion(quat_desired.v(0),quat_desired.v(1),quat_desired.v(2),quat_desired.a);
  ...
  //// calculate new pos in global frame
   ...
   Eigen::VectorXd tau(7),tau_filt(7);
