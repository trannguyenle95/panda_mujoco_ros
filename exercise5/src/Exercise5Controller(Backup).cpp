...
void Exercise5Controller::update(const ros::Time &time, const ros::Duration &period) {
    ...
    Fd << 0.0, 0.0, -2.0, 0.0, 0.0, 0.0;

    Eigen::Matrix<double, 7, 1> tau;
    tau.setZero();

    // todo:  Compute effort (tau) such that cartesian force is constant
...
Eigen::Affine3d Exercise5Controller::dkt(const std::array<double, 7> &q, const Eigen::Vector3d &r) const {
